...
build-combine-base:
	docker build -t combine-base --build-arg SPARK_VERSION=2.4.4 --build-arg HADOOP_VERSION_SHORT=2.7 --build-arg ELASTICSEARCH_HADOOP_CONNECTOR_VERSION=7.2.1 combine_base

build-combine-hadoop:
	docker build -t combine-hadoop --build-arg COMBINE_BRANCH=dev --build-arg LIVY_TAGGED_RELEASE=0.5.0-incubating --build-arg SCALA_VERSION=2.11 --build-arg HADOOP_VERSION=$(HADOOP_VERSION) combine_hadoop
